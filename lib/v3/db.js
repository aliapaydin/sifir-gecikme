...
export async function initDb() {
  ...
  // Tema tercihi (light | dark | system) — kullanıcıya özel, cihazlar arası taşınır
  await sql`ALTER TABLE v3_users ADD COLUMN IF NOT EXISTS theme TEXT`;
  ...
  // İçerik görüntülenme sayıları (günlük, anonim)
  await sql`
    CREATE TABLE IF NOT EXISTS v3_content_views (
      href       TEXT NOT NULL,
      day        DATE NOT NULL DEFAULT CURRENT_DATE,
      count      INTEGER NOT NULL DEFAULT 1,
      PRIMARY KEY (href, day)
    )
  `;
  await sql`CREATE INDEX IF NOT EXISTS v3_content_views_href_idx ON v3_content_views(href)`;
  await sql`CREATE INDEX IF NOT EXISTS v3_content_views_day_idx  ON v3_content_views(day)`;

  // Kullanıcı verisi: genel JSONB blob (origin/main uyumu)
  await sql`
    CREATE TABLE IF NOT EXISTS v3_user_data (
      user_id    UUID PRIMARY KEY REFERENCES v3_users(id) ON DELETE CASCADE,
