...
const FROM = process.env.RESEND_FROM || 'Sıfır Gecikme <onboarding@resend.dev>';
...
<table width="100%" style="max-width:560px;">
  ...
  <tr><td style="padding-bottom:24px;text-align:center;">
    <a href="https://www.sifirgecikme.com" style="text-decoration:none;font-size:22px;font-weight:800;background:linear-gradient(135deg,#6366f1,#8b5cf6,#14b8a6);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;color:#6366f1;">◈ Sıfır Gecikme v3</a>
  </td></tr>
  ...
  <tr><td style="padding-top:24px;text-align:center;font-size:12px;color:#334155;line-height:1.6;">
    ...
    <a href="https://www.sifirgecikme.com" style="color:#4f46e5;text-decoration:none;">sifirgecikme.com</a>
  </td></tr>
...
export async function sendVerificationEmail({ to, name, token, origin }) {
  const verifyUrl = `${origin}/dogrula?token=${token}`;
  ...
  const content = `
    ...
    <tr><td style="padding:0 36px 8px;">
      ...
      ${sectionCard('🐍', 'Python Playground', 'Tarayıcıda gerçek Python — Pyodide ile kurulum yok, anında çalıştır.', `${origin}/python`, 'Playground\'a git')}
      ${sectionCard('📚', 'Öğrenme Modülü', '20 ders, 315 XP · Python, NumPy, Pandas, İstatistik — Türkçe ve ücretsiz.', `${origin}/ogren`, 'Öğrenmeye başla')}
      ${sectionCard('🗄️', 'SQL Playground', '3 farklı veritabanı, şema gezgini — tarayıcıda SQLite, kurulum yok.', `${origin}/sql`, 'SQL\'i dene')}
      ${sectionCard('🤖', 'Kalori AI', 'Fotoğrafından kalori tahmini — Gemini Vision ile anlık analiz.', `${origin}/kalori`, 'Dene')}
      ${sectionCard('🖥️', 'Tech Center', 'Sanal bilgisayar mağazası simülasyonu — müşteri, stok, kredi sistemi.', `${origin}/tech-center`, 'Oyna')}
    </td></tr>
...
export async function sendNewsletterEmail({ to, name, subject, articles, origin }) {
  const content = `
    <!-- Gradient bar -->
    <tr><td style="height:4px;background:linear-gradient(90deg,#6366f1,#8b5cf6,#14b8a6);"></td></tr>
    ...
    <tr><td style="padding:36px 36px 0;text-align:center;">
      <div style="font-size:40px;margin-bottom:16px;">📬</div>
      <h1 style="margin:0 0 12px;font-size:24px;font-weight:800;color:#f1f5f9;line-height:1.2;">Merhaba, ${name}!</h1>
      <p style="margin:0;font-size:15px;color:#64748b;line-height:1.6;">
        Bu ay Sıfır Gecikme'den öne çıkan içerikler seni bekliyor.
      </p>
    </td></tr>
    ...
    <!-- Articles -->
    <tr><td style="padding:0 36px 8px;">
      <p style="margin:0 0 16px;font-size:12px;font-weight:700;letter-spacing:1px;text-transform:uppercase;color:#475569;">Bu Ay Öne Çıkanlar</p>
      ${articles.map(a => sectionCard(
        a.badge === 'interaktif' ? '🔬' : a.badge === 'araç' ? '🛠️' : a.badge === 'kariyer' ? '💼' : a.badge === 'vaka çalışması' ? '📊' : '📚',
        a.baslik,
        a.ozet,
        `${origin}${a.href}`,
        'Okumaya devam et'
    ...
    <tr><td style="padding:8px 36px 36px;text-align:center;">
      ...
      ${ctaButton(`${origin}/icerikler`, '📚 Tüm İçerikleri Gör')}
    </td></tr>
  `;
