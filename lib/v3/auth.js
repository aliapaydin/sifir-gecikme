  ...
  try {
    ...
    const [session] = await sql`
      SELECT s.id, s.expires_at, u.id as user_id, u.name, u.email, u.role, u.avatar_color,
             ...
             u.patreon_id, u.patreon_name, u.is_supporter, u.patron_status,
             u.pledge_cents, u.lifetime_cents, u.patreon_linked_at
      FROM v3_sessions s
    ...
    return {
      ...
      patreonId: session.patreon_id,
      patreonName: session.patreon_name,
      isSupporter: session.is_supporter || false,
      patronStatus: session.patron_status,
      pledgeCents: session.pledge_cents || 0,
      lifetimeCents: session.lifetime_cents || 0,
      patreonLinkedAt: session.patreon_linked_at,
    };
  } catch {
    return null;
  }
