...
export async function getSession(request) {
  ...
  try {
    ...
    // Patreon + tema alanları henüz tabloda olmayabilir — ayrı sorguda dene
    try {
      const [p] = await sql`
        SELECT patreon_id, patreon_name, is_supporter, patron_status,
               pledge_cents, lifetime_cents, patreon_linked_at, theme
      ...
      if (p) {
        ...
        base.theme           = p.theme ?? null;
      }
    } catch { /* Patreon/tema kolonları henüz yok, sorun değil */ }
...
export async function deleteSession(sessionId) {
  ...
  const cookieStore = await cookies();
  const domain = await getCookieDomain();
  // Host-only (eski) cookie'yi temizle
  cookieStore.delete(COOKIE);
  // Domain-kapsamlı (paylaşılan) cookie'yi de temizle
  if (domain) {
