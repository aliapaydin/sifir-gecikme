...
export function generateOneCustomer(state, unlockedCats) {
  ...
  // Kurumsal bütçe: ürün maliyeti × miktar × makul kar marjı (statik aralık pahalı ürünlerde yetersiz kalıyor)
  let budget;
  if (typeKey === 'corporate') {
    const unitBuyPrice = getEffectiveBuyPrice(wantedProduct.id, state.activeEvents || []);
    budget = Math.round(quantity * unitBuyPrice * randFloat(1.10, 1.35));
  } else {
    budget = randInt(ctype.budgetRange[0], ctype.budgetRange[1]);
  }
