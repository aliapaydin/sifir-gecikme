...
export function generateOneCustomer(state, unlockedCats) {
  ...
  const weights = typeKeys.map(k => {
    const base = CUSTOMER_TYPES[k].frequency;
  ...
  // Toplu alım (kurumsal)
  let quantity = 1;
  if (ctype.bulkMin && ctype.bulkMax) {
...
export function processDayEnd(state) {
  ...
  newState = {
    ...
    skippedCount: 0,
    daySummary,
    gamePhase: firmaValue >= WIN_TARGET ? 'won' : 'day_summary',
    wonOnDay: firmaValue >= WIN_TARGET ? newState.currentDay : newState.wonOnDay,
  };
...
export function placeOrder(state, productId, quantity, supplier) {
  ...
  // Mark matching shopping list items as ordered
  const updatedShoppingList = (state.shoppingList || []).map(item =>
    item.productId === productId && !item.ordered ? { ...item, ordered: true } : item
  );
...
export function delayCustomer(state, customerId, days) {
  ...
  const acceptChance = days === 1 ? 0.70 : 0.50;
  ...
  if (accepted) {
    ...
    const updatedCustomers = state.customersToday.map(c =>
      c.id === customerId ? { ...c, dealt: true, result: 'delayed' } : c
    );
    return {
      ...state,
      customersToday: updatedCustomers,
