...
export function useTechCenterState() {
  ...
  // Yükle — localStorage öncelikli, DB yalnızca boşsa (yeni cihaz)
  useEffect(() => {
    async function load() {
      let localState = null;
      try {
      ...
      try {
        ...
        if (res.ok) {
          const data = await res.json();
          if (data?.user) isLoggedIn.current = true;
        }
      } catch {}

      if (localState) {
        // Yerel veri var → onu kullan, arka planda DB'yi güncelle
        setStateRaw(localState);
        setLoaded(true);
        if (isLoggedIn.current) saveToDb(localState);
      } else if (isLoggedIn.current) {
        // Yerel yok → farklı cihaz, TC-spesifik DB'den dene
        try {
    ...
    // UserDataProvider'ın v3_user_data'dan localStorage'ı güncellemesi bekleniyor.
    // SYNC_READY_EVENT gelince localStorage'dan yeniden yükle (daha güncel olabilir).
    function onSyncReady() {
      try {
        const raw = localStorage.getItem(STORAGE_KEY);
        if (!raw) return;
        const synced = JSON.parse(raw);
        // Mevcut state'ten daha ileri ise uygula
        setStateRaw(prev => {
          if (!prev) return synced;
          const prevProgress = (prev.currentDay || 0) * 100000 + (prev.allTimeSales?.length || 0);
          const syncProgress = (synced.currentDay || 0) * 100000 + (synced.allTimeSales?.length || 0);
          return syncProgress > prevProgress ? synced : prev;
        });
      } catch {}
    }

    window.addEventListener(SYNC_READY_EVENT, onSyncReady);
    return () => window.removeEventListener(SYNC_READY_EVENT, onSyncReady);
  }, []);
  ...
  const setState = useCallback((updater) => {
    setStateRaw(prev => {
      ...
      if (isLoggedIn.current) {
        ...
        dbSaveTimer.current = setTimeout(() => saveToDb(next), 5000);
      }
      return next;
    });
  }, []);
